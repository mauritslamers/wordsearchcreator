...
void PageLayoutDialog::PageSizeChanged()
{
    ...
    {
        float pageWidth = PageLayoutDialog::paperSize(static_cast<QPrinter::PageSize>(a)).width;
        float pageHeight = PageLayoutDialog::paperSize(static_cast<QPrinter::PageSize>(a)).height;
        if(
            (
                (qRound(pageWidth)==qRound(widthSpinBox->value()*72.0))
                    &&
                (qRound(pageHeight)==qRound(heightSpinBox->value()*72.0)))
            ||
                ((qRound(pageWidth)==qRound(heightSpinBox->value()*72.0))
                    &&
                (qRound(pageHeight)==qRound(widthSpinBox->value()*72.0))))
        {
